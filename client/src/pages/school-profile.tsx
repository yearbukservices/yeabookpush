...
import { ViewerDashboardLayout } from "@/components/ViewerDashboardLayout";
import { BookOpen, Award, Image as ImageIcon, MapPin, Globe, Calendar, Share2, ArrowLeft } from "lucide-react";
...
export default function SchoolProfile() {
  ...
  const handleInteractiveAction = (action: string, targetUrl?: string) => {
    if (!user) {
  ...
  const isSchoolOwner = user?.userType === 'school' && user?.schoolId === profileData?.school.id;
  ...
  const profileContent = (
    <>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              {user && user.userType === 'viewer' && (
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => handleInteractiveAction('yearbooks', '/yearbook-finder')}
          ...
          <div className="mt-8">
            <div className="bg-gradient-to-r from-cyan-500/20 to-purple-500/20 backdrop-blur-lg rounded-2xl p-6 border border-white/20 shadow-2xl">
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                ...
                <Button
                  onClick={() => handleInteractiveAction('alumni', '/request-alumni-status')}
      ...
      {/* Login/Signup Modal for Unregistered Users */}
  ...
  if (user && user.userType === 'viewer') {
    return (
      <ViewerDashboardLayout user={user}>
        {profileContent}
